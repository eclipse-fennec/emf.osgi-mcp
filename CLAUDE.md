...
| `org.eclipse.fennec.mcp.inference.config` | Configurator JSON for the inference feature: `MCPToolProvider~inference` (20) and the `/mcp/inference` server with its own `auth.token` and `server.instructions`. It deliberately carries **no** `ModelAtlasPublisher` config: that PID is owned by `model.atlas.mcp.config` in the model.atlas project, which `inference.runtime` requires by identity. Two configs for one factory PID would activate two publishers, and `PostToModelAtlasTool`'s unary reference would bind an arbitrary one. Its properties come from `MODEL_ATLAS_*` / `MCP_ATLAS_PUBLISH_ALLOWLIST` (env, or system properties via `secrets.bndrun`). |
