...
| `org.eclipse.fennec.mcp.api` | **The server half.** Depends on `mcp.endpoint`. `MCPServer` (extends `MCPEndpoint`, adds the aggregated tool/prompt/resource specs), `MCPTool`, `MCPToolProvider`, `MCPResourceProvider`, `MCPPromptProvider`. Abstract bases: `AbstractMCPTool` (schema loading), `AbstractHttpMCPServer` (servlet lifecycle). Bind to `MCPEndpoint` to merely reach a server, `MCPServer` to read what it serves. Also the one place both tool bundles can share: `UriPatterns` (the exact / `prefix*` / `*` matcher every allow- and deny-list speaks) and `AnnotationVisibility` + its `MCPAnnotationVisibility` PID, which is here rather than in either tool bundle so an annotation deny-list cannot be half-enforced. This is the only bundle in `api` carrying a DS component. |
...
| `org.eclipse.fennec.mcp.emf.tools` | EMF model tools (28 MCP tools): build/validate/serialize EMF instances **and** author Ecore metamodels (EPackage/EClass/EDataType/EEnum/EAnnotation/EOperation/features + full generics), register them into a session-local `PackageRegistry` (so their classes become instantiable), and round-trip via hardened inline-XMI import. `create_epackage` and `add_eclass` are **composite**: they take their children inline (`eClassifiers` / `eAttributes` / `eReferences` / `eAnnotations` / `eLiterals`), wired in a second pass so intra-package `#//<Name>` refs are order-independent, all-or-nothing against the dataset, and returning the `objectId` of every nested element — an agent authors a package in one call instead of one per element. `export_package` emits a registered package's full `.ecore` (annotations, abstract classes, nsURI-based hrefs for cross-package supertypes) — the full-fidelity read `describe_eclass` cannot give. `list_metamodel`, `describe_eclass` and `export_package` also take a `fingerprint`: they read through an `EPackage.Registry` (one package per nsURI), so without it they are blind to a second registered version rather than merely ambiguous — the fingerprint resolves via `MetadataService` and reaches a version the registry does not hold, an nsURI with several versions is refused, and `export_package` refuses a version mismatch outright. Session-scoped datasets with replayable recipes. Deny-all allow-lists on `EMFModelGuard` PID (exact / `prefix*` / `*` patterns via `UriPatterns`, shared with `EMFPackageRegistry`; `allowedPackages()` filters the live registry so late-registered packages match a prefix rule, and the package and class lists stay independent), registration policy + LRU cap on `EMFPackageRegistry` PID, caps on `EMFDatasetRegistry` PID. See `docs/emf-metamodel-authoring.md`. |
| `org.eclipse.fennec.mcp.metadata.tools` | Discovery tools (9) over the EMF metadata layer (`MetadataService` / `MetadataIndexReader`): query classes/features/operations by EAnnotation across every registered package (omitting `value` matches any value for the key), resolve a bare class name, enumerate annotation sources and aspect type ids, read one element's parsed aspects with their diagnostics, and report index wiring. **Every nsURI-addressed read also takes a `fingerprint`**, because registration is keyed by model fingerprint and one nsURI can hold several versions (one model at two atlas stages): an ambiguous nsURI is refused via `PackageSelector`, never resolved to the newest, and the refusal lists the fingerprints (`describe_metadata_status` lists them unprompted). Hits carry `modelFingerprint` and de-duplicate per version, so a wide query stops collapsing two versions into one. `origin` is three-way and read positively — `osgi-service` (has `service.id`), `session` (announced with no properties), `external` (registered with build context by a third party, e.g. model.atlas staged reads) — because the old two-way split attributed anything without a `service.id` to the caller's own session. Identity-and-structure results, no package/class allow-list — "query wide to locate, read narrow to copy" — but the `MCPAnnotationVisibility` deny-list **is** enforced here (a source denied only in `emf.tools` would still be enumerated by `list_annotation_sources` and returned by `find_*_by_annotation`). No codec dependency. See `docs/metadata-discovery-tools.md`. |
...
| `org.eclipse.fennec.mcp.emf.runtime.config.atlas` | Optional overlay pointing the runtime at a model.atlas scope for **reading** EPackages (`rest.client~jena`); needs the `model.atlas.rest.client.*` bundles. Read side only — publishing config lives in the model.atlas project's `model.atlas.mcp.config`. `base.uri` is `$[env:MODEL_ATLAS_BASE_URI]`, deliberately the same variable the publisher reads, so one value configures both directions. |
| `org.eclipse.fennec.mcp.inference.runtime` | **Resolution anchor for the inference feature.** Provides `osgi.implementation=mcp.inference` and requires the closure it needs: `emf.tools`, `metadata.tools`, `inference.config`, and the model.atlas project's `model.atlas.mcp.tools` + `model.atlas.mcp.config`. One `-runrequires` on the capability turns the feature on, which is what makes `~inference`'s hard minimum of 20 safe. Its `MCPServer` reference is targeted at `(server.name=osgi-emf-inference-mcp-server)` — a runtime hosting both endpoints publishes more than one. Has `launch.bndrun` + `secrets.bndrun.template` for both tokens, and `inference.runtime_docker.bndrun` — the same requirements resolved for a container, exported by `export.inference.runtime_docker` into `docker/inference/` (see that folder's README). Its `-dependson` lists every bundle the bndruns name only by identity: without it an export resolves against the previously built copy, so a changed configurator JSON ships stale. |
| `org.eclipse.fennec.mcp.inference.config` | Configurator JSON for the inference feature: `MCPToolProvider~inference` (20) and the `/mcp/inference` server with its own `auth.token` and `server.instructions`. It deliberately carries **no** `ModelAtlasPublisher` config: that PID is owned by `model.atlas.mcp.config` in the model.atlas project, which `inference.runtime` requires by identity. Two configs for one factory PID would activate two publishers, and `PostToModelAtlasTool`'s unary reference would bind an arbitrary one. Its properties come from `MODEL_ATLAS_*` / `MCP_ATLAS_PUBLISH_ALLOWLIST` (env, or system properties via `secrets.bndrun`). `runtime.json`'s bind host is `$[env:MCP_INFERENCE_HTTP_HOST;default=127.0.0.1]` — loopback locally, `0.0.0.0` in the docker image, which is also why a token is mandatory there: bound to a container interface, no caller is loopback any more and a tokenless endpoint answers 401 to everything. |
...
- **GitHub Actions**: six of the seven workflows are thin callers into the org-wide reusables in [`eclipse-fennec/.github`](https://github.com/eclipse-fennec/.github), SHA-pinned with the version tag as a trailing comment — change that CI logic there, not here. The **one exception** is `reusable-container.yml`, which is repo-local and does carry inline steps: there is no org-wide container reusable, and event.atlas set the same precedent.
  - `build.yml` (all branches except main/snapshot + PRs) → `reusable-verify`, with `export.inference.runtime_docker` as an `extra-gradle-tasks` so a PR proves the docker bndrun still exports (an export fails when a `-runbundle` cannot be found). Needs `gradle-parallel: false` — bnd resolve/export is not parallel-safe in this workspace.
  - `snapshot.yml` (snapshot branch) → `reusable-verify` → `reusable-release` (`do-release: false`) → `reusable-container` (`docker-label: snapshot`) + `reusable-docs`
  - `release.yml` (main branch) → the same chain with `do-release: true` (GPG-signed, deploys to Central Sonatype) and `docker-label: latest`
  ...
  - `reusable-verify` runs the license header check as the first gating job, then `./gradlew clean build testOSGi` on the Java 21 + 25 matrix and `./gradlew perfTest`. Secrets reach only `reusable-release` and `reusable-container`, via `secrets: inherit`.
- **Container images**: `reusable-container.yml` pushes to `docker.io/eclipsefennec/emf.osgi-mcp` and `ghcr.io/eclipse-fennec/emf.osgi-mcp` for `linux/amd64` + `linux/arm64/v8`. This repo can produce more than one runtime image, so **the variant lives in the tag**, not in the repository name: `emf.osgi-mcp:inference-snapshot` and `:inference-<bundle version>` (the way model.atlas tags `model.atlas:jena-snapshot`). Every path is derived from the `variant` input by naming convention — `docker/<variant>/`, `org.eclipse.fennec.mcp.<variant>.runtime`, `<variant>.runtime_docker.jar` — so a second image is one more caller job, not a second workflow. The jar is **not** rebuilt there: `reusable-release` exports it via `extra-gradle-tasks` and uploads it as the `release-jars` artifact, so the image embeds exactly the jar that was published to Maven. Inside the container job both jars are located **by file name** under the download directory, never by rebuilding the path they were uploaded from: `actions/upload-artifact` strips the least common ancestor of the paths it is given, so that layout depends on how many projects the caller happened to list (with a single project it flattens to `distributions/executable/<jar>`) and changes the moment a second variant is added. Requires the `DOCKER_USERNAME` / `DOCKER_API_TOKEN` secrets and `packages: write` on the calling workflow.
