...
- **Container images**: `reusable-container.yml` pushes to `docker.io/eclipsefennec/emf.osgi-mcp` and `ghcr.io/eclipse-fennec/emf.osgi-mcp` for `linux/amd64` + `linux/arm64/v8`. This repo can produce more than one runtime image, so **the variant lives in the tag**, not in the repository name: `emf.osgi-mcp:inference-snapshot` and `:inference-<bundle version>` (the way model.atlas tags `model.atlas:jena-snapshot`). Every path is derived from the `variant` input by naming convention — `docker/<variant>/`, `org.eclipse.fennec.mcp.<variant>.runtime`, `<variant>.runtime_docker.jar` — so a second image is one more caller job, not a second workflow. The jar is **not** rebuilt there: `reusable-release` exports it via `extra-gradle-tasks` and uploads it as the `release-jars` artifact, so the image embeds exactly the jar that was published to Maven. Requires the `DOCKER_USERNAME` / `DOCKER_API_TOKEN` secrets and `packages: write` on the calling workflow.
